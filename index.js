...
app.get("/contacts/search", async (req, res) => {
  try {
    ...
    if (!q || q.trim().length < 2) {
      ...
      const contacts = (data.documents || []).map(parseDoc).filter(isPersonContact);
  ...
  try {
    ...
    while (loaded < 5000) {
      ...
      for (const doc of data.documents) {
        const c = parseDoc(doc);
        if (!isPersonContact(c)) continue;
        sample.push({ id: c.id, name: c.name || `${c.firstName || ""} ${c.lastName || ""}`.trim(), company: c.company || "", jobTitle: c.jobTitle || "", metroArea: c.metroArea || "", industry: c.industry || "", importGroups: c.importGroups || "", email: c.email || "", status: c.status || "" });
        loaded++;
      }
      if (!data.nextPageToken) break;
      token = data.nextPageToken;
    ...
    const claudeRes = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({ model: "claude-sonnet-4-20250514", max_tokens: 2000, messages: [{ role: "user", content: `You are a CRM assistant. Based on this request: "${prompt}"\n\nFind the best matching contacts. Return ONLY a JSON array of contact IDs, max 50.\nFormat: ["id1", "id2", ...]\n\nContacts:\n${JSON.stringify(sample)}\n\nReturn ONLY the JSON array.` }] })
    });
    const claudeData = await claudeRes.json();
    const text = claudeData.content?.[0]?.text || "[]";
    const ids = JSON.parse(text.replace(/```json|```/g, "").trim());
    res.json({ contacts: sample.filter(c => ids.includes(c.id)), total: ids.length, prompt });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.get("/dashboard/stats", async (req, res) => {
  try {
    const [emails, tasks, meetings] = await Promise.all([fetchAll("emails"), fetchAll("tasks"), fetchAll("meetings")]);
    const now = Date.now();
    const DAY = 86400000;
    ...
    for (const sent of sentEmails) {
      ...
      const replied = emails.some(e => e.contactId === sent.contactId && e.direction === "received" && new Date(e.date) > new Date(sent.date));
    ...
    res.json({ totalEmails: emails.length, totalTasks: tasks.length, pendingTasks: tasks.filter(t => t.status === "pending").length, totalMeetings: meetings.length, upcomingMeetings: meetings.filter(m => m.status === "upcoming").length, followUps: followUps.slice(0, 20) });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
  ...
  try {
    ...
    while (true) {
      ...
      for (const doc of data.documents) {
        ...
        if (isPersonContact(c)) companyMap[co].contacts.push({ id: c.id, name: c.name || `${c.firstName || ""} ${c.lastName || ""}`.trim(), jobTitle: c.jobTitle || "", email: c.email || "", phone: c.phone || "" });
      }
      if (!data.nextPageToken) break;
      token = data.nextPageToken;
      await new Promise(r => setTimeout(r, 30));
    }
    const companies = Object.values(companyMap).filter(co => co.contacts.length > 0).sort((a, b) => a.name.localeCompare(b.name));
    ...
    while (true) {
      ...
      for (const doc of data.documents) {
        ...
        gRaw.split(",").map(g => g.trim()).filter(Boolean).forEach(g => {
          ...
          grpMap[g].push({ id: c.id, name: c.name || `${c.firstName || ""} ${c.lastName || ""}`.trim(), company: c.company || "", email: c.email || "", jobTitle: c.jobTitle || "" });
        });
      }
      if (!data.nextPageToken) break;
      token = data.nextPageToken;
